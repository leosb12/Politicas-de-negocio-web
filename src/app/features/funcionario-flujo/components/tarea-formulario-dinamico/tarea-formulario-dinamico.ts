...
import {
  ...
  FlujoArchivoMetadata,
  FlujoFormularioCampo,
  FlujoFormularioDefinicion,
...
export class TareaFormularioDinamicoComponent {
  ...
  control(campo: FlujoFormularioCampo): FormControl<DynamicControlValue> {
  ...
  onFileSelected(campo: FlujoFormularioCampo, event: Event): void {
  ...
  clearFile(campo: FlujoFormularioCampo): void {
    this.control(campo).setValue(null);
    this.control(campo).markAsTouched();
  }

  selectedFile(campo: FlujoFormularioCampo): FlujoArchivoMetadata | null {
  ...
  isPendingUpload(campo: FlujoFormularioCampo): boolean {
    return this.control(campo).value instanceof File;
  }

  fieldError(campo: FlujoFormularioCampo): string | null {
  ...
  private rebuildForm(
    definition: FlujoFormularioDefinicion | null,
  ...
  private toLocalFileMetadata(file: File): FlujoArchivoMetadata {
    return {
      archivoId: null,
      nombre: file.name,
  ...
  private isArchivoMetadata(value: unknown): value is FlujoArchivoMetadata {
    if (!value || typeof value !== 'object') {
      return false;
    }

    const candidate = value as Partial<FlujoArchivoMetadata>;
    return (
