...
import { isFormularioTipoSoportado } from './funcionario-flujo-status.util';
...
): FlujoFormularioCampoTipo | null {
  if (!isFormularioTipoSoportado(fieldType)) {
    return null;
  }

  return fieldType.trim().toUpperCase() as FlujoFormularioCampoTipo;
}
