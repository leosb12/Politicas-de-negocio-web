...
import {
  classifyFlujoConflict,
  mapFlujoUiError,
} from './funcionario-flujo-error.util';
import {
  mapInstanciaDetalleDto,
  mapTareaDetalleDto,
  mapTareaMiaDto,
} from './funcionario-flujo.mapper';
import {
  isTareaCompletable,
  isTareaTomable,
  normalizeEstado,
} from './funcionario-flujo-status.util';
import { FuncionarioFlujoApiService } from './funcionario-flujo-api.service';

@Injectable({
  providedIn: 'root',
})
export class FuncionarioFlujoFacadeService {
  ...
  readonly instanciaPageError = signal<FlujoUiError | null>(null);
    ...
    this.api
      ...
      .pipe(
        ...
        catchError((error: unknown) => {
          const mappedError = mapFlujoUiError(
            error,
  ...
  private toFlujoArchivoMetadata(
  ...
  private handleTaskActionError$(
    ...
    const mappedError = mapFlujoUiError(error, fallbackMessage);
    this.detalleError.set(mappedError);
  ...
  private applyConflictRules(rawMessage: string | null, fallbackMessage: string): void {
    const conflictType = classifyFlujoConflict(rawMessage);
