...
import {
  AdministradorRolFormularioComponent,
  AdministradorRolFormularioValue,
} from '../../components/administrador-rol-formulario/administrador-rol-formulario';
import {
  AdministradorRol,
...
@Component({
  selector: 'app-administrador-roles-page',
  imports: [
    CommonModule,
    ReactiveFormsModule,
    DataTableComponent,
  ...
  templateUrl: './administrador-roles.html',
  styleUrl: './administrador-roles.css',
})
export class AdministradorRolesPageComponent {
  ...
  readonly selectedRole = signal<AdministradorRol | null>(null);
  ...
  openEditRole(role: AdministradorRol): void {
  ...
  saveRole(formValue: AdministradorRolFormularioValue): void {
    this.roleFormPending.set(true);
    this.roleFormError.set(null);

    let request$: Observable<AdministradorRol>;
    let successMessage = '';

    if (this.roleFormMode() === 'create') {
      const payload: CreateAdministradorRolRequest = {
      ...
      const payload: UpdateAdministradorRolRequest = {
        descripcion: formValue.descripcion.length > 0 ? formValue.descripcion : null,
  ...
  requestToggleRoleStatus(role: AdministradorRol): void {
  ...
  requestDeleteRole(role: AdministradorRol): void {
    if (role.sistema) {
      return;
    }
  ...
  private toggleRoleStatus(role: AdministradorRol): void {
    const shouldDeactivate = role.activo;
  ...
  private deleteRole(role: AdministradorRol): void {
    this.confirmPending.set(true);
    this.actionLoadingRoleId.set(role.id);
