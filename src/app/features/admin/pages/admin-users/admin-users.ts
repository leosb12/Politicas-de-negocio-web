...
import {
  ...
  AdminUserFormValue,
} from '../../components/admin-user-form/admin-user-form';
...
export class AdminUsersPageComponent {
  ...
  readonly viewedUser = signal<AdminUser | null>(null);
  readonly viewUserError = signal<string | null>(null);

  readonly assignRoleOpen = signal(false);
  readonly assignRoleUser = signal<AdminUser | null>(null);
  ...
  saveUser(formValue: AdminUserFormValue): void {
    this.userFormPending.set(true);
    this.userFormError.set(null);

    let request$: Observable<AdminUser>;
    let successMessage = '';
  ...
  openViewUser(user: AdminUser): void {
  ...
  requestToggleUserStatus(user: AdminUser): void {
  ...
  openAssignRole(user: AdminUser): void {
  ...
  requestRemoveRole(user: AdminUser): void {
    if (user.rol === 'USUARIO') {
      return;
    }
  ...
  private toggleUserStatus(user: AdminUser): void {
    const shouldDeactivate = user.activo;
  ...
  private buildCreatePayload(formValue: AdminUserFormValue): CreateAdminUserRequest {
    return {
  ...
  private buildUpdatePayload(formValue: AdminUserFormValue): UpdateAdminUserRequest {
