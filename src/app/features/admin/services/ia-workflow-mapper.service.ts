...
export interface IaWorkflowMapperContext {
...
export class IaWorkflowMapperService {
   ...
   */
  mapIaResponseToWorkflow(
    iaResponse: IaWorkflowResponse,
    context?: IaWorkflowMapperContext
  ...
  private mapNodes(
    ...
    const rolesById = new Map<string, IaRole>(
  ...
  private buildFormsByNodeIdMap(forms: IaForm[]): Map<string, IaForm> {
    const map = new Map<string, IaForm>();
  ...
  ): Map<string, IaTransition[]> {
    const map = new Map<string, IaTransition[]>();
  ...
  private isInitiatorResponsibility(iaNode: IaNode, role: IaRole | null): boolean {
   ...
   */
  private mapTransitions(iaTransitions: IaTransition[]): Conexion[] {
   ...
   */
  private mapNodeType(iaType: IaNodeType): TipoNodo {
    const typeMap: Record<IaNodeType, TipoNodo> = {
      start: 'INICIO',
      task: 'ACTIVIDAD',
      decision: 'DECISION',
      parallel_start: 'FORK',
   ...
   */
  private mapFieldType(iaFieldType: IaFieldType): TipoCampo {
    const fieldMap: Partial<Record<IaFieldType, TipoCampo>> = {
      text: 'TEXTO',
      textarea: 'TEXTO',
      email: 'TEXTO',
      phone: 'TEXTO',
  ...
  private mapFormFields(form: IaForm | null): CampoFormulario[] {
  ...
  private mapDecisionConditions(
    iaNode: IaNode,
    outgoingByNodeId: Map<string, IaTransition[]>,
    incomingByNodeId: Map<string, IaTransition[]>,
    iaNodes: IaNode[],
    formsByNodeId: Map<string, IaForm>
  ): CondicionDecision[] {
