...
import {
  ...
  EmployeeGuideScreen,
} from '../../../funcionario-workflow/models/employee-guide.model';
import { EmployeeGuideContextService } from '../../../funcionario-workflow/services/employee-guide-context.service';
import { EmployeeGuideService } from '../../../funcionario-workflow/services/employee-guide.service';

type GuideBotRole = 'ADMIN' | 'FUNCIONARIO';
type GuideBotSeverity = 'INFO' | 'WARNING' | 'ERROR' | 'SUCCESS';
type GuideBotResponse = AdminGuideResponse | EmployeeGuideResponse;
...
@Component({
  selector: 'app-admin-guide-bot',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule],
  templateUrl: './admin-guide-bot.html',
  styleUrl: './admin-guide-bot.css',
})
export class AdminGuideBotComponent {
  ...
  private readonly adminGuideContext = inject(AdminGuideContextService);
  private readonly adminGuideService = inject(AdminGuideService);
  private readonly employeeGuideContext = inject(EmployeeGuideContextService);
  private readonly employeeGuideService = inject(EmployeeGuideService);
  ...
  adminResponse(message: GuideChatMessage): AdminGuideResponse | null {
    const response = message.response;
    if (!response || !('detectedIssues' in response) || !('suggestedForm' in response)) {
      return null;
    }
    return response as AdminGuideResponse;
  }

  employeeResponse(message: GuideChatMessage): EmployeeGuideResponse | null {
    const response = message.response;
    if (!response || !('formHelp' in response) || !('missingFields' in response)) {
      return null;
    }
    return response as EmployeeGuideResponse;
  }
  ...
  private mapUrlToAdminScreen(url: string): AdminGuideScreen {
  ...
  private mapUrlToEmployeeScreen(url: string): EmployeeGuideScreen {
  ...
  private buildAdminEmergencyFallback(question: string): AdminGuideResponse {
    const screen = this.adminGuideContext.screen();
